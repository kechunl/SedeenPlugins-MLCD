...
PythonKernel::PythonKernel(wchar_t* pyHOME, std::string path, std::vector<std::string> csv_paths):
	pathsToFeatures_(csv_paths),
	PYTHONHOME(pyHOME)
	{
...
std::string PythonKernel::doProcessData()
{
	...
	try
	{
		...
		// load the module
		std::shared_ptr<sedeen::python::PythonObject> pModule(sedeen::python::PythonObject::py_ImportModule(pyModuleName_.c_str())); // "Sedeen_Diagnosis"
		...
		// find the class
		std::string python_class_name_ = "Diagnosis";
		...
		PyObject* pROIs = sedeen::python::PythonObject::pyobject_from(pathsToFeatures_)->py_Object();
        PyObject * pArgs = Py_BuildValue("(O)", pROIs);
        assert(pArgs != NULL);
	...
	catch (std::exception& e)
	{
		...
		std::string logname = strcat(stamp, "_Diagnosis_Exception.txt");
		// Full path
