...
namespace sedeen {
  namespace python {
    ...
    PythonObject::PythonObject(const char* pVal)
    {
      ...
      PyObject_ = PyUnicode_FromString(pVal);
      assert(PyObject_ != NULL);
      decRef_ = true;
    }
    ...
    void PythonObject::py_SetTupleItem(size_t pos, PythonObject& obj)
    {
      // set the tuple item
      ...
      PythonObject::py_ReprVal() const
    {
      ...
      return PyUnicode_AsUTF8(repr.py_Object());
    }
    ...
    {
      return new PythonObject(PyLong_FromLong(int(value)), true);
    }
    ...
    {
      return new PythonObject(PyUnicode_FromString(value.empty() ? "" : value.c_str()), true);
    }

    PythonObject* PythonObject::pyobject_from(const sedeen::ColorSpace& value)
    {
      //Not implemented
      return new PythonObject(PyLong_FromLong(0), true);
    }
    ...
    PythonObject* PythonObject::pyobject_from(const std::vector<std::string>& value)
    {
        PyObject* listObj = PyList_New(value.size());
        if (listObj == NULL)
            return nullptr;
        for (size_t i = 0; i < value.size(); i++) {
            PyObject* strObj = PyUnicode_FromString(value[i].empty() ? "" : value[i].c_str());
            if (strObj == NULL) {
                Py_DECREF(listObj);
                return nullptr;
            }
            PyList_SET_ITEM(listObj, i, strObj);
        }
        return new PythonObject(listObj, true);
    }
