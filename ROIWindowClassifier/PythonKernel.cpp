...
PythonKernel::PythonKernel(wchar_t* pyHOME, std::string path, std::string image_path, std::string output_path):
	pathToImage_(image_path),
	pathToSaveFile_(output_path),
	PYTHONHOME(pyHOME)
	{
...
std::string PythonKernel::doProcessData()
{
	...
	try
	{
		...
		// load the module
		std::shared_ptr<sedeen::python::PythonObject> pModule(sedeen::python::PythonObject::py_ImportModule(pyModuleName_.c_str())); // "Sedeen_ROIWindowClassifier"
		assert(pModule->py_Object() != NULL);

		// find the class
		std::string python_class_name_ = "ROIWindowClassifier";
        ...
        PyObject * pArgs = Py_BuildValue("(s, s)", pathToImage_.c_str(), pathToSaveFile_.c_str());
        assert(pArgs != NULL);
        ...
        bool conversionIsDone = sedeen::python::PythonObject::pyobject_to(pResult->py_Object(), bboxfilepath);
	...
	catch (std::exception& e)
	{
		...
		std::string logname = strcat(stamp, "_ROIWindowClassifier_Exception.txt");
		// Full path
